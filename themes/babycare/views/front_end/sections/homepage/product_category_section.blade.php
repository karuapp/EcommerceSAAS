                        ...
                        <div class="product-content-top text-center">
                            ...
                            <h4><a href="{{ url($slug.'/product/'.$items->slug) }}">{{ $items->name }}</a> </h4>
                                <div class="favorite-icon">
                                    ...
                                    {!! \App\Models\Product::actionLinks($currentTheme, $slug, $items) !!}
                                </div>
                        </div>
                        <div class="product-img">
                            <a href="{{ url($slug.'/product/'.$items->slug) }}">
                                <img src="{{ get_file($items->cover_image_path,$currentTheme) }}" alt="product-img">
                            </a>
                        </div>
                        <div class="product-content-bottom text-center">
                            <div class="price-label2">
                                @if ($items->variant_product == 0)
                                    <div class="price">
                                    {!! \App\Models\Product::getProductPrice($items, $store, $currentTheme) !!}
                                    </div>
                                @else
                                    <div class="price">
                            ...
                            {!! \App\Models\Product::ProductcardButton($currentTheme, $slug, $items) !!}
                            </div>
                        </div>
                    </div>
                </div>
