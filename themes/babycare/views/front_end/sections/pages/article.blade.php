...
@section('content')
    ...
    <section class="blog-page-banner article-page-banner common-banner-section" style="background-image: url({{ get_file( $page_json->article_page->section->image->image ?? 'themes/' .$currentTheme.'/assets/images/banner-inner.png')}});">
        <div class="container">
                    ...
                    <div class="common-banner-content text-center">
                        ...
                        <div class="about-user d-flex align-items-center">
                            <div class="abt-user-img">
                                <img src="{{ !empty($user->profile_image) ? asset($user->profile_image) : asset(Storage::url('uploads/profile/avatar.png')) }}" alt="profile">
                            </div>
                            <h6>
                                <span>{{ $user->name }},</span>
                                 {{-- {{ __('company.com') }} --}}
                                ...
                                @for ($i = 0; $i < $section->footer->section->footer_link->loop_number ?? 1; $i++)
                                    <li>
                                        <a href="{{ $section->footer->section->footer_link->social_link->{$i} ?? '#'}}" target="_blank">
                                            <img src="{{ get_file($section->footer->section->footer_link->social_icon->{$i}->image ?? 'themes/' . $currentTheme . '/assets/img/youtube.png', $currentTheme) }}"  alt="icon">
                                        </a>
                                    </li>
                                @endfor
                            @endif
                                ...
                                <div class="blog-inner">
                                    <div class="blog-img">
                                        <a href="{{route('page.article',[$slug,$data->id])}}">
                                            <img src="{{ get_file($data->cover_image_path, $currentTheme) }}" alt="blog-img">
                                        </a>
                                    </div>
