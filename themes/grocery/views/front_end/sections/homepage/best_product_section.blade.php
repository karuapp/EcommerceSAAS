...
style="position: relative;@if(isset($option) && $option->is_hide == 1) opacity: 0.5; @else opacity: 1; @endif" data-index="{{ $option->order ?? '' }}" data-id="{{ $option->order ?? '' }}" data-value="{{ $option->id ?? '' }}" data-hide="{{ $option->is_hide ?? '' }}" data-section="{{ $option->section_name ?? '' }}"  data-store="{{ $option->store_id ?? '' }}" data-theme="{{ $option->theme_id ?? '' }}">
    ...
    <div class="container">
        <div class="section-title d-flex align-items-center justify-content-between">
            ...
            <a href="{{ route('page.product-list',  ['storeSlug' => $slug]) }}" class="btn" id="{{ $section->best_product->section->button->slug ?? '' }}_preview">
        ...
        <div class="row row-gap">
                    <div class="col-xl-6 col-lg-5 col-md-4 col-12">
                        <div class="category-card">
                            <div class="category-img">
                                <img src="{{ get_file($section->best_product->section->image->image ?? '', $currentTheme) }}"
                ...
                {!! $section->best_product->section->description->text ?? '' !!}
                                ...
                                <div class="btn-wrapper">
                                    <a href="{{ route('page.product-list', $slug) }}" class="btn" id="{{ $section->best_product->section->button->slug ?? '' }}_preview">
                {!! $section->best_product->section->button->text ?? '' !!}<svg xmlns="http://www.w3.org/2000/svg"
