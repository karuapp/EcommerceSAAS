                    ...
                    <tbody>
                        ...
                        @foreach ($order['product'] as $product)
                            <tr class="service">
                                ...
                                <td class="tableitem">
                                    @if ($order['paymnet_type'] == 'POS')
                                        {{ currency_format_with_sym($product['final_price'] ?? 0, getCurrentStore(), APP_THEME()) ?? SetNumberFormat($product['orignal_price']) }}
                                    @else
                                        @if (module_is_active('ProductPricing') && isset($product['sale_price']))
                                            {{ currency_format_with_sym($product['sale_price'] ?? 0, getCurrentStore(), APP_THEME()) ?? SetNumberFormat($product['sale_price']) }}
                                        @else
                                            {{ currency_format_with_sym($product['final_price'] ?? 0, getCurrentStore(), APP_THEME()) ?? SetNumberFormat($product['final_price']) }}
                                        @endif
                                    @endif
                                </td>
                            </tr>
                        ...
                        <tr class="tabletitle">
                            ...
                            <td class="payment">
                                <h2><b>{{ currency_format_with_sym($product['final_price'] ?? 0, getCurrentStore(), APP_THEME()) ?? SetNumberFormat($order['sub_total']) }}</b>
                                </h2>
                            </td>
                        </tr>
                    </tbody>
        ...
        <div class="footer pt-2">
            <center>
                <p class="text-dark p-2">
                    {{ __('Merchandise may not be returned for refund at any time. Power bases cannot be
