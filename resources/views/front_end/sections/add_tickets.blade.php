{{ Form::open(['route' => ['support.ticket.store',$slug], 'method' => 'post', 'enctype' => 'multipart/form-data']) }}
    <div class="row">
        ...
        <div class="modal-footer pb-0">
            <button type="button" value="{{__('Cancel')}}" class="btn btn-badge btn-secondary cancel-btn" data-bs-dismiss="modal">{{__('Cancel')}}</button>
            <button type="submit" value="{{__('Create')}}" class="btn btn-badge btn-primary mx-1 continue-btn">{{__('Create')}}</button>
        </div>
    </div>
{!! Form::close() !!}
