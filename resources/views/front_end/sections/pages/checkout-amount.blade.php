...
@endphp
@stack('ManageDepositAmountValue')
<div class="order-paycol-inner">
    <div class="d-flex align-items-center justify-content-between payment-ttl-row">
        <div class="payment-ttl-left">
            ...
            <span>
                {{ __('Sub-total') }}:
                <b> {{ currency_format_with_sym(($response->data->final_price ?? 0), $store->id, $currentTheme) ?? SetNumberFormat($response->data->final_price ?? ($response->data->sub_total ?? 0)) }}</b>
            </span>
            <span>
                {{ __('Tax') }} :
                <b>
        ...
        <div class="payment-ttl-left">
            ...
            <div class="ttl-amount">
                ...
                <div class="ttl-pric final_amount_currency1 shipping_total_price">
                    {{ currency_format_with_sym(($response->data->final_price ?? 0), $store->id, $currentTheme) ?? SetNumberFormat($response->data->final_price) }}
                </div>
            </div>
        </div>
    </div>
