    ...
    <div id="cart-body" class="mini-cart-has-item">
                    ...
                    <div class="mini-cart-item">
                        ...
                        <div class="mini-cart-details">
                            ...
                            <div class="qty-spinner">
                                ...
                                <input type="text" class="quantity 45_quatity" data-cke-saved-name="quantity" name="quantity" value="{{ $item->qty }}" min="01" id="cart_quantity{{ $item->qty }}" data-id="463" disabled>
                            ...
                            <div class="pvarprice d-flex align-items-center justify-content-between">
                                <div class="price">
                                    {!! \App\Models\Product::ManageCheckoutPrice($item, $store, $store->theme_id) !!}
                                </div>
                                <button class="btn remove_item_from_cart" title="Remove item" data-id="{{ $item->cart_id }}" data-product-id = "{{ $item->product_id }}"  data-variant-id = "{{ $item->variant_id }}">
        ...
        <div class="mini-cart-footer">
            ...
            <div class="cart-amount-section">
                ...
                <div class="u-save d-flex justify-end" >
                    {{ __('Total') }} :  <span class="final_amount_currency shipping_total_price" final_total="{{ $response->data->total_sub_price  }}">{{ currency_format_with_sym(($response->data->total_sub_price ?? 0), $store->id, $currentTheme) ?? SetNumberFormat($response->data->total_sub_price) }}</span>
                </div>
            </div>

            @include('front_end.hooks.checkout_list')
            @if (isset($default_checkout_btn) && $default_checkout_btn == 'on')
