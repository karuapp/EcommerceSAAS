...
class SupportTicketDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        ...
        ->editColumn('status', function (SupportTicket $ticket) {
            return '<span class="badge fix_badges bg-primary p-2 px-3 rounded">'.$ticket->status.'</span>';
        })
    ...
    {
        return $model->newQuery()->leftjoin('customers', 'customers.id', '=', 'support_tickets.customer_id')->where('support_tickets.theme_id', APP_THEME())->where('support_tickets.store_id', getCurrentStore())->select('support_tickets.*', 'customers.first_name', 'customers.last_name'); ;
    }
    ...
    public function html(): HtmlBuilder
    {
        $dataTable = $this->builder()
            ->setTableId('supportticket-table')
        ...
        if (module_is_active('BulkDelete')) {
            $bulkdeleteButtonConfig = bulkDeleteForm('supportticket','supportticket-table');
        }
    ...
    public function getColumns(): array
    {
        return [
            ...
            Column::make('customer_id')->title(__('User')),
