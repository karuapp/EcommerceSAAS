...
class Utility extends Model
{
    ...
    public static function Seting($theme_id = null, $store_id = null)
    {
        ...
        $defaults = [
            ...
            'sms_setting_enabled' => '',
            'theme_image' => 'themes/' . ($theme_id ?? APP_THEME()) . '/theme_img/img_1.png'
        ...
        if (\Auth::check()) {
            ...
            $data = Cache::rememberForever('auth_user_setting_' . $user->id, function () use ($conditions) {
                return Setting::where($conditions)->get();
            });

            // If no settings are found, fallback to default settings for non-admin users
            if (count($data) == 0) {
                $data = Cache::rememberForever('auth_user_setting_' . $user->id, function () {
                    return DB::table('settings')->where('created_by', 1)->get();
                });
            }
            ...
            if ($theme_id !== null && $store_id !== null) {
                // Fetch settings based on the provided store_id and theme_id
                $data = Cache::rememberForever("default_setting_{$store_id}_{$theme_id}", function () use ($store_id, $theme_id) {
                    return Setting::where('store_id', $store_id)
                        ->where('theme_id', $theme_id)
                        ->get();
                });
            } else {
                // Fallback to default settings for unauthenticated users
                $data = Cache::rememberForever('default_setting_1_grocery', function () {
                    return Setting::where('created_by', 1)
                        ->where('store_id', 1)
                        ->where('theme_id', 'grocery')
                        ->get();
                });
            }
    ...
    {
        // Set the theme_id and store_id if not provided
        ...
        if (!empty($name)) {
            ...
            $cacheKey = "settings_store_{$store_id}_theme_{$theme_id}";

            // Retrieve the settings from cache or store them if not already cached
            $settings = Cache::rememberForever($cacheKey, function () use ($theme_id, $store_id) {
                // Initialize the query
                $query = Setting::query();

                // Apply the theme_id condition only if it's not empty
                if (!empty($theme_id)) {
            ...
            if (isset($settings[$name])) {
                ...
                $utility_setting = Self::Seting($theme_id, $store_id);
    ...
    public static function GetValByName($name = '', $theme_id = '', $store_id = '')
    {
        ...
        if (!empty($name)) {
            ...
            // Retrieve the settings from cache or store them if not already cached
            $settings = Cache::rememberForever($cacheKey, function () use ($name, $theme_id, $store_id) {
                // Initialize the query
                $query = Setting::where('name', $name);

                // Apply theme_id condition if it's not empty
                if (!empty($theme_id)) {
                    $query->where('theme_id', $theme_id);
                }
    ...
    public static function themeSubcategory($theme_id = null, $store_id = null)
    {
        if (empty($theme_id)) {
            $theme_id = APP_THEME();
        }
        ...
        if (!empty($store_id) && !empty($theme_id)) {
            $count = SubCategory::where('store_id', $store_id)->where('theme_id', $theme_id)->count();
            if ($count > 0) {
                $return = 1;
            }
        }
    ...
    public static function payment_data($payment = '', $theme_id = '', $store_id = '')
    {
        ...
        //CinetPay
        if ($payment == 'CinetPay') {
            $is_cinetpay_enabled = \App\Models\Utility::GetValueByName('is_cinetpay_enabled', $theme_id, $store_id);
            $cinet_pay_unfo = \App\Models\Utility::GetValueByName('cinet_pay_unfo', $theme_id, $store_id);
            $cinet_pay_site_id = \App\Models\Utility::GetValueByName('cinet_pay_site_id', $theme_id, $store_id);
            $cinet_pay_image = \App\Models\Utility::GetValueByName('cinet_pay_image', $theme_id, $store_id);
            $cinet_pay_api_key = \App\Models\Utility::GetValueByName('cinet_pay_api_key', $theme_id, $store_id);
            $cinet_pay_secret_key = \App\Models\Utility::GetValueByName('cinet_pay_secret_key', $theme_id, $store_id);
        ...
        if ($payment == 'SenagePay') {
            $is_Senangpay_enabled = Utility::GetValueByName('is_Senangpay_enabled', $theme_id);
            $senang_pay_image = Utility::GetValueByName('senang_pay_image', $theme_id);
            $senang_pay_unfo = Utility::GetValueByName('senang_pay_unfo', $theme_id);
            $Senangpay_mode = Utility::GetValueByName('Senangpay_mode', $theme_id);
            $senang_pay_merchant_id = Utility::GetValueByName('senang_pay_merchant_id', $theme_id);
            $senang_pay_secret_key = Utility::GetValueByName('senang_pay_secret_key', $theme_id);
        ...
        if ($payment == 'CyberSource') {
            $is_cybersource_enabled = Utility::GetValueByName('is_cybersource_enabled', $theme_id);
            $cybersource_pay_image = Utility::GetValueByName('cybersource_pay_image', $theme_id);
            $cybersource_pay_unfo = Utility::GetValueByName('cybersource_pay_unfo', $theme_id);
            $cybersource_pay_merchant_id = Utility::GetValueByName('cybersource_pay_merchant_id', $theme_id);
            $cybersource_pay_secret_key = Utility::GetValueByName('cybersource_pay_secret_key', $theme_id);
            $cybersource_pay_api_key = Utility::GetValueByName('cybersource_pay_api_key', $theme_id);
        ...
        //ozow
        if ($payment == 'Ozow') {
            $is_ozow_enabled = Utility::GetValueByName('is_ozow_enabled', $theme_id);
            $ozow_pay_image = Utility::GetValueByName('ozow_pay_image', $theme_id);
            $ozow_pay_unfo = Utility::GetValueByName('ozow_pay_unfo', $theme_id);
            $ozow_mode = Utility::GetValueByName('ozow_mode', $theme_id);
            $ozow_pay_Site_key = Utility::GetValueByName('ozow_pay_Site_key', $theme_id);
            $ozow_pay_private_key = Utility::GetValueByName('ozow_pay_private_key', $theme_id);
            $ozow_pay_api_key = Utility::GetValueByName('ozow_pay_api_key', $theme_id);
        ...
        if ($payment == 'MyFatoorah') {
            $is_myfatoorah_enabled = Utility::GetValueByName('is_myfatoorah_enabled', $theme_id, $store_id);
            $myfatoorah_pay_image = Utility::GetValueByName('myfatoorah_pay_image', $theme_id, $store_id);
            $myfatoorah_pay_unfo = Utility::GetValueByName('myfatoorah_pay_unfo', $theme_id, $store_id);
            $myfatoorah_mode = Utility::GetValueByName('myfatoorah_mode', $theme_id, $store_id);
            $myfatoorah_pay_country_iso = Utility::GetValueByName('myfatoorah_pay_country_iso', $theme_id, $store_id);
            $myfatoorah_pay_api_key = Utility::GetValueByName('myfatoorah_pay_api_key', $theme_id, $store_id);
        ...
        // easebuzz
        if ($payment == 'easebuzz') {
            $is_easebuzz_enabled = Utility::GetValueByName('is_easebuzz_enabled', $theme_id);
            $easebuzz_image = Utility::GetValueByName('easebuzz_image', $theme_id);
            $easebuzz_unfo = Utility::GetValueByName('easebuzz_unfo', $theme_id);
            $easebuzz_enviroment_name = Utility::GetValueByName('easebuzz_enviroment_name', $theme_id);
            $easebuzz_merchant_key = Utility::GetValueByName('easebuzz_merchant_key', $theme_id);
            $easebuzz_salt_key = Utility::GetValueByName('easebuzz_salt_key', $theme_id);
        ...
        if ($payment == 'NMI') {
            ...
            $is_nmi_enabled = Utility::GetValueByName('is_nmi_enabled', $theme_id);
            $nmi_image = Utility::GetValueByName('nmi_image', $theme_id);
            $nmi_unfo = Utility::GetValueByName('nmi_unfo', $theme_id);
            $nmi_api_private_key = Utility::GetValueByName('nmi_api_private_key', $theme_id);
        ...
        //PayU
        if ($payment == 'PayU') {
            $is_payu_enabled = \App\Models\Utility::GetValueByName('is_payu_enabled', $theme_id, $store_id);
            $payu_unfo = \App\Models\Utility::GetValueByName('payu_unfo', $theme_id, $store_id);
            $payu_merchant_key = \App\Models\Utility::GetValueByName('payu_merchant_key', $theme_id, $store_id);
            $payu_salt_key = \App\Models\Utility::GetValueByName('payu_salt_key', $theme_id, $store_id);
            $payu_image = \App\Models\Utility::GetValueByName('payu_image', $theme_id, $store_id);
            $payu_mode = \App\Models\Utility::GetValueByName('payu_mode', $theme_id, $store_id);
        ...
        // Sofort
        if ($payment == 'sofort') {
            $is_sofort_enabled = \App\Models\Utility::GetValueByName('is_sofort_enabled', $theme_id, $store_id);
            $sofort_publishable_key = \App\Models\Utility::GetValueByName('sofort_publishable_key', $theme_id, $store_id);
            $sofort_secret_key = \App\Models\Utility::GetValueByName('sofort_secret_key', $theme_id, $store_id);
            $sofort_image = \App\Models\Utility::GetValueByName('sofort_image', $theme_id, $store_id);
        ...
        // ESewa
        if ($payment == 'esewa') {
            $is_esewa_enabled = \App\Models\Utility::GetValueByName('is_esewa_enabled', $theme_id, $store_id);
            $esewa_merchant_key = \App\Models\Utility::GetValueByName('esewa_merchant_key', $theme_id, $store_id);
            $esewa_mode = \App\Models\Utility::GetValueByName('esewa_mode', $theme_id, $store_id);
            $esewa_image = \App\Models\Utility::GetValueByName('esewa_image', $theme_id, $store_id);
        ...
        //Paynow
        if ($payment == 'Paynow') {
            $is_paynow_enabled = Utility::GetValueByName('is_paynow_enabled', $theme_id, $store_id);
            $paynow_pay_image = Utility::GetValueByName('paynow_pay_image', $theme_id, $store_id);
            $paynow_pay_unfo = Utility::GetValueByName('paynow_pay_unfo', $theme_id, $store_id);
            $paynow_mode = Utility::GetValueByName('paynow_mode', $theme_id, $store_id);
            $paynow_pay_integration_id = Utility::GetValueByName('paynow_pay_integration_id', $theme_id, $store_id);
            $paynow_pay_integration_key = Utility::GetValueByName('paynow_pay_integration_key', $theme_id, $store_id);
            $paynow_pay_merchant_email = Utility::GetValueByName('paynow_pay_merchant_email', $theme_id, $store_id);
        ...
        //DPO
        if ($payment == 'DPO') {
            $is_dpopay_enabled = Utility::GetValueByName('is_dpopay_enabled', $theme_id, $store_id);
            $dpo_pay_image = Utility::GetValueByName('dpo_pay_image', $theme_id, $store_id);
            $dpo_pay_unfo = Utility::GetValueByName('dpo_pay_unfo', $theme_id, $store_id);
            $dpo_pay_Company_Token = Utility::GetValueByName('dpo_pay_Company_Token', $theme_id, $store_id);
            $dpo_pay_Service_Type = Utility::GetValueByName('dpo_pay_Service_Type', $theme_id, $store_id);
        ...
        if ($payment == 'Braintree') {
            $is_braintree_enabled = Utility::GetValueByName('is_braintree_enabled', $theme_id, $store_id);
            $braintree_pay_image = Utility::GetValueByName('braintree_pay_image', $theme_id, $store_id);
            $braintree_pay_unfo = Utility::GetValueByName('braintree_pay_unfo', $theme_id, $store_id);
            $braintree_mode = Utility::GetValueByName('braintree_mode', $theme_id, $store_id);
            $braintree_pay_merchant_id = Utility::GetValueByName('braintree_pay_merchant_id', $theme_id, $store_id);
            $braintree_pay_public_key = Utility::GetValueByName('braintree_pay_public_key', $theme_id, $store_id);
            $braintree_pay_private_key = Utility::GetValueByName('braintree_pay_private_key', $theme_id, $store_id);
        ...
        if ($payment == 'PowerTranz') {
            $is_powertranz_enabled = Utility::GetValueByName('is_powertranz_enabled', $theme_id, $store_id);
            $powertranz_pay_image = Utility::GetValueByName('powertranz_pay_image', $theme_id, $store_id);
            $powertranz_pay_unfo = Utility::GetValueByName('powertranz_pay_unfo', $theme_id, $store_id);
            $powertranz_mode = Utility::GetValueByName('powertranz_mode', $theme_id, $store_id);
            $powertranz_pay_production_url = Utility::GetValueByName('powertranz_pay_production_url', $theme_id, $store_id);
            $powertranz_pay_merchant_id = Utility::GetValueByName('powertranz_pay_merchant_id', $theme_id, $store_id);
            $powertranz_pay_processing_password = Utility::GetValueByName('powertranz_pay_processing_password', $theme_id, $store_id);
        ...
        if ($payment == 'SSLCommerz') {
            $is_sslcommerz_enabled = Utility::GetValueByName('is_sslcommerz_enabled', $theme_id, $store_id);
            $sslcommerz_pay_image = Utility::GetValueByName('sslcommerz_pay_image', $theme_id, $store_id);
            $sslcommerz_pay_unfo = Utility::GetValueByName('sslcommerz_pay_unfo', $theme_id, $store_id);
            $sslcommerz_mode = Utility::GetValueByName('sslcommerz_mode', $theme_id, $store_id);
            $sslcommerz_pay_store_id = Utility::GetValueByName('sslcommerz_pay_store_id', $theme_id, $store_id);
            $sslcommerz_pay_secret_key = Utility::GetValueByName('sslcommerz_pay_secret_key', $theme_id, $store_id);
    ...
    public static function replaceVariables($content, $obj, $store, $order, $user_id = 0)
    {
        ...
        // Abandon Cart
        $cart = Cart::where('customer_id', $user_id)->where('theme_id', $store->theme_id)->get();
        if (!$cart->isEmpty() && !is_array($obj)) {
            ...
            foreach ($obj as $item) {
                if (isset($item->product_id)) {
                    ...
                    $listItems .= '<td>' . '<img src="' . get_file($product->cover_image_path, $store->theme_id) . '" height="60" width="80" ">' . '</td>';
        ...
        $Wishlist = Wishlist::where('customer_id', $user_id)->where('theme_id', APP_THEME())->get();
        if (!$Wishlist->isEmpty()) {
            // Abandon  Wishlist
            ...
            foreach ($obj as $item) {
                ...
                $wishItems .= '<td>' . '<img src="' . get_file($product->cover_image_path, $store->theme_id) . '" height="60" width="80" ">' . '</td>';
    ...
    public static function SendMsgs($template, $mobile_no, $msg = '')
    {
        ...
        if (getCurrentStore()) {
            ...
            $theme_name = $store->theme_id ?? (!empty(APP_THEME()) ? APP_THEME() : env('DATA_INSERT_APP_THEME'));
        } else {
        ...
        $whatstemplate = WhatsappMessage::where('name', 'LIKE', $template)->where('theme_id', $theme_name)->where('store_id', $store->id)->first();
    ...
    public static function affiliateTransaction($order, $customer = '')
    {
        ...
        $useradmin = User::where('current_store', $objUser->store_id)->where('theme_id', $objUser->theme_id)->first();
        $affiliateSetting = AffiliateSetting::where('store_id', $objUser->store_id)->where('theme_id', $objUser->theme_id)->first();
        if ($objUser->used_affiliate_code != 0 && $user == null && (isset($affiliateSetting) && $affiliateSetting->is_enable == 1)) {
            ...
            $transaction->theme_id            = $objUser->theme_id;
