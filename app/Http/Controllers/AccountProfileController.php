...
class AccountProfileController extends Controller
{
    ...
    public function states_list(Request $request, $slug)
    {
        ...
        $state_list = State::orderBy('name','ASC')->where('country_id', $country_id)->pluck('name', 'id')->prepend('Select State', 0)->toArray();
    ...
    public function city_list(Request $request, $slug)
    {
        ...
        $city_list = City::where('state_id', $state_id)->orderBy('name','ASC')->pluck('name', 'id')->prepend('Select City', '0')->toArray();
    ...
    public function wish_list(Request $request, $slug)
    {
		...
		if (auth('customers')->user()) {
            ...
            $currency_icon = Utility::GetValueByName('CURRENCY', $store->theme_id, $store->id);
            ...
            $errors=[];
                 foreach($request->file('attachments') as $filekey => $file)
                {
                    ...
                    if($result==1)
                    {
                        ...
                        $dir        = 'themes/'.$theme_id.'/uploads/tickets';
    ...
    public function order_refund_request(Request $request, $slug, $id)
    {
        ...
        $order_refund->theme_id          =  $store->theme_id;

        $data              = [];
        if ($request->hasfile('attachments')) {
            foreach ($request->file('attachments') as $filekey => $file) {
                $imageName = $file->getClientOriginalName();
                $dir        = 'themes/' . $store->theme_id . '/uploads/order_refund';
                $path = Utility::keyWiseUpload_file($request, 'attachments', $imageName, $dir, $filekey, []);
    ...
    public function change_refund_cart(Request $request, $slug)
    {
        ...
        $CURRENCY = \App\Models\Utility::GetValueByName('CURRENCY', APP_THEME(), getCurrentStore());
        $final_price = 0;
        ...
        $return['product_price'] = currency_format_with_sym( $final_price, getCurrentStore(), APP_THEME()) ?? SetNumberFormat($final_price);
        $return['CURRENCY'] = $CURRENCY;
        $return['tax_price'] = currency_format_with_sym( $order['tax_price'], getCurrentStore(), APP_THEME()) ?? SetNumberFormat($order['tax_price']);
        $return['discount_price'] = currency_format_with_sym( (($order['coupon_info']) ?  $order['coupon_info']['discount_amount'] : 0), getCurrentStore(), APP_THEME()) ?? SetNumberFormat(($order['coupon_info']) ?  $order['coupon_info']['discount_amount'] : 0);
        $return['delivered_charge'] = currency_format_with_sym( $order['delivered_charge'], getCurrentStore(), APP_THEME()) ?? SetNumberFormat($order['delivered_charge']);
