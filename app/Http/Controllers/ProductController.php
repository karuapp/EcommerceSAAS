...
class ProductController extends Controller
{
    ...
    public function index(Request $request,ProductDataTable $dataTable)
    {
        if (auth()->user() && auth()->user()->isAbleTo('Manage Products')) {
            $products = Product::where('theme_id', APP_THEME())->where('product_type', null)->where('store_id', getCurrentStore())->orderBy('id', 'desc')->get();
    ...
    public function searchProductsSku(Request $request)
    {
        ...
        if ($request->ajax() && isset($lastsegment) && !empty($lastsegment)) {
            ...
            if ($request->cat_id !== '' && $request->search == '') {
                ...
                if ($request->cat_id == '0') {
                    $products = Product::where('slug', 'LIKE', "%{$request->search}%")->where('store_id', getCurrentStore())->where('theme_id', $store_id->theme_id)->get();
                } else {
                    $products = Product::where('slug', 'LIKE', "%{$request->search}%")->where('store_id', getCurrentStore())->where('theme_id', $store_id->theme_id)->Where('maincategory_id', $request->cat_id)->get();
                }
            }
            if (count($products) > 0) {
                foreach ($products as $key => $product) {
    ...
    {
        $store = $store_id = Cache::remember('store_' . getCurrentStore(), 3600, function () {
            return Store::where('id', getCurrentStore())->first();
        });
        ...
        $carthtml .= '<tr data-product-id="' . $id . '" id="product-id-' . $id . '">
                        ...
                        <td class="">
                            ...
                            <button type="button" class="show_confirm btn btn-sm btn-danger" data-confirm="' . __('Are You Sure?') . '"
data-text="' . __('This action can not be undone. Do you want to continue?') . '">
    ...
    public function getProductsVariantQuantity(Request $request)
    {
        ...
        if ($product && $request->variants != '') {
            ...
            if ($variant) {
                ...
                $price = currency_format_with_sym(Product::ProductPrice($store->theme_id, $store->slug, $product->id,$variant->id));
    ...
    {
        if(auth()->user() && auth()->user()->isAbleTo('Manage Products')) {
            $products = Product::where('theme_id', APP_THEME())->where('product_type', null)->where('store_id', getCurrentStore())->orderBy('id', 'desc')->paginate(11);
            $settings = Setting::where('theme_id', APP_THEME())->where('store_id', getCurrentStore())->pluck('value', 'name')->toArray();
