...
class OrderController extends Controller
{
    ...
    public function order_view(Request $request, $id)
    {
        if (auth()->user() && auth()->user()->isAbleTo('Show Order')) {
            try {
                $id = Crypt::decrypt($id);
                $order = Order::order_detail($id);
                $store_id = $store = Cache::remember('store_' . getCurrentStore(), 3600, function () {
                ...
                $deliveryboys = DeliveryBoy::where('theme_id', $store_id->theme_id)->where('store_id', $store_id->id)->pluck('name', 'id')->prepend('Assign to delivery boy', "");
    ...
    public function applycoupon(Request $request, $slug)
    {
        ...
        $coupon = Coupon::whereRaw('BINARY `coupon_code` = ?', [$request->coupon_code])->where('theme_id', $request->theme_id)->where('store_id', $store->id)->first();
    ...
    public function order_summary(Request $request, $slug)
    {
        ...
        if (session('data')) {
            ...
            $storeUrl = $this->getFrontThemeUrl($store);
            $url = $storeUrl.'/' . $slug . '/order/' . \Illuminate\Support\Facades\Crypt::encrypt($orders_data->id ?? '');
            return view('front_end.sections.pages.order-summary', compact('slug', 'url', 'store', 'order_id', 'orders_data', 'currentTheme', 'currantLang', 'currency', 'languages', 'section', 'order_complate_title', 'order_complate_description') + $sqlData + $data);
            ...
            $storeUrl = $this->getFrontThemeUrl($store);
            $url =  $storeUrl .'/'. $slug . '/order/' . \Illuminate\Support\Facades\Crypt::encrypt($order_data->id ?? '');
            return view('front_end.sections.pages.order-summary', compact('store','url', 'order_data', 'slug', 'currentTheme', 'currantLang', 'currency', 'languages', 'section', 'order_complate_description', 'order_complate_title', 'MainCategoryList') + $sqlData + $data);
        } else {
    ...
    public function order_grid_view(Request $request)
    {
        if(auth()->user() && auth()->user()->isAbleTo('Manage Products')) {
            $orders = Order::where('theme_id', APP_THEME())->where('store_id', getCurrentStore())->orderBy('created_at', 'desc')->paginate(16);
