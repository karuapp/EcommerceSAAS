...
class MenuController extends Controller
{
    ...
    public function edit(Menu $menu)
    {
        if(auth()->user() && auth()->user()->isAbleTo('Edit Menu'))
        {
            ...
            if (isset($menu->id)) {
                ...
                $brands      = ProductBrand::where('status', 1)->where('theme_id', APP_THEME())->where('store_id',getCurrentStore())->pluck('name', 'id');
                return view('menu.edit', compact('categories', 'desiredMenu', 'menuitems','pages','brands'));
            }
        }
        else
        {
    ...
    public function addBrandToMenu(Request $request)
    {
        ...
        if ($menu->content == '' || empty($menu->content) || $menu->content == 'null') {
            $array = [];
            foreach ($ids as $key => $id) {
                $title      = ProductBrand::where('id', $id)->value('name');
                $slug       = ProductBrand::where('id', $id)->value('slug');
                $item = MenuItem::create([
                    'title'         => $title,
